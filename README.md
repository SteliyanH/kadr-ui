...
See [ROADMAP.md](ROADMAP.md) for KadrUI's own milestones (shipped: v0.8.0 SpeedCurveEditor / CaptionEditor / OverlayInspectorPanel + OverlayKeyframeEditor; next: v1.0 stability — DocC tutorials, snapshot tests), and [Kadr's ROADMAP](https://github.com/SteliyanH/kadr/blob/main/ROADMAP.md) for the upstream library. KadrUI ships on its own version track but each release is gated on the matching Kadr public surface.
